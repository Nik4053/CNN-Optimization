...
__global__
void cb_padded_norelu_reorder_pos(Tensor<float, 4> errorPadded, Tensor<float, 4> next_error, Tensor<float,4> outputPadded,Tensor<float, 4> weights) {
	...
	for (int c_out = 0; c_out < output_channels; c_out++) {
		...
		#pragma unroll
		for (int b = 0; b < batch_size; b++) {
			long err_pos = errorPadded.pos(b, c_out, y, x);
			float val = 0.;
			for (int j = 0; j < filter_size; j++) {
				for (int i = 0; i < filter_size; i++) {
					// float error_val = errorPadded(b, c_out, y + j, x + i);
					float error_val = errorPadded(err_pos + i);
					val += error_val * reg_weights[j][i];
				}
				err_pos += errorPadded.getDim(2);
			}
			reg_vals[b] +=val;
		}
	}
...
template __global__ void cb_padded_norelu_reorder_pos<3,1>(Tensor<float, 4> error, Tensor<float, 4> next_error, Tensor<float,4> output,Tensor<float, 4> weights);
template __global__ void cb_padded_norelu_reorder_pos<3,10>(Tensor<float, 4> error, Tensor<float, 4> next_error, Tensor<float,4> output,Tensor<float, 4> weights);
template __global__ void cb_padded_norelu_reorder_pos<3,100>(Tensor<float, 4> error, Tensor<float, 4> next_error, Tensor<float,4> output,Tensor<float, 4> weights);