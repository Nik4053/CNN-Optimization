...
template<int filter_size = 3, int batch_size>
__global__
void cb_padded_norelu_reorder_save(Tensor<float, 4> errorPadded, Tensor<float, 4> next_error, Tensor<float,4> outputPadded,Tensor<float, 4> weights) {
	const int yBatch = 3;
	...
	for (int c_out = 0; c_out < output_channels; c_out++) {
		...
		for (int b = 0; b < batch_size; b++) {
			float val[yBatch];
			val[0] += errorPadded(b, c_out, y + 0, x + 0) * reg_weights[0][0];
			val[0] += errorPadded(b, c_out, y + 0, x + 1) * reg_weights[0][1];
			val[0] += errorPadded(b, c_out, y + 0, x + 2) * reg_weights[0][2];

			val[0] += errorPadded(b, c_out, y + 1, x + 0) * reg_weights[1][0];
			val[0] += errorPadded(b, c_out, y + 1, x + 1) * reg_weights[1][1];
			val[0] += errorPadded(b, c_out, y + 1, x + 2) * reg_weights[1][2];



			val[0] += errorPadded(b, c_out, y + 2, x + 0) * reg_weights[2][0];
			val[0] += errorPadded(b, c_out, y + 2, x + 1) * reg_weights[2][1];
			val[0] += errorPadded(b, c_out, y + 2, x + 2) * reg_weights[2][2];
			// for (int j = 0; j < filter_size; j++) {
			// 	for (int i = 0; i < filter_size; i++) {
...
template __global__ void cb_padded_norelu_reorder_save<3,1>(Tensor<float, 4> error, Tensor<float, 4> next_error, Tensor<float,4> output,Tensor<float, 4> weights);
template __global__ void cb_padded_norelu_reorder_save<3,10>(Tensor<float, 4> error, Tensor<float, 4> next_error, Tensor<float,4> output,Tensor<float, 4> weights);
template __global__ void cb_padded_norelu_reorder_save<3,100>(Tensor<float, 4> error, Tensor<float, 4> next_error, Tensor<float,4> output,Tensor<float, 4> weights);