...
void w_padded(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights) {
	...
	const short batch_size = inputPadded.getDim(0),
				input_channels = inputPadded.getDim(1),
				output_channels = error.getDim(1),
				height = error.getDim(2),
				width = error.getDim(3);

	if( x >= width || y >= height || z >= gradient_weights.getDim(0) * gradient_weights.getDim(1))
		return;
	...
	for (short j = 0; j < filter_size; j++) {
		for (short i = 0; i < filter_size; i++) {
			...
			for (short b = 0; b < batch_size; b++) {
				...
				float input_val =0;
				input_val=inputPadded(b, c_in, y + j , x + i); // for each image go through the same filter for one single channel
...
template __global__ void  w_padded<3>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);