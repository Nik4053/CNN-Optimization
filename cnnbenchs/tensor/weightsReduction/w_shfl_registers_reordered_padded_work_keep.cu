...
void w_shfl_registers_reordered_padded_work_keep(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights) {
	...
	float reuse_vals[filter_size][filter_size];
	...
	for (int b = 0; b < batch_size; b++) {
		for(int j=1;j<filter_size;j++){
			for(int i=0;i<filter_size;i++){
				reuse_vals[j][i]=inputPadded(b, c_in, y - 1 + j, x + i);
			}
		}
		#pragma unroll
		for ( int yy = 0; yy < yBatch; yy++){
			...
			// for(int j=0;j<filter_size;j++){
			// 	for(int i=0;i<filter_size;i++){
			// 		reuse_vals[j][i]=inputPadded(b, c_in, y + yy + j, x + i);
			// 	}
			// }
			for (int j = 0; j < filter_size-1; j++) {
				for (int i =0; i < filter_size; i++) {
					reuse_vals[j][i] = reuse_vals[j+1][i];	
				}	
			}
			for(int i=0;i<filter_size;i++){
				reuse_vals[filter_size-1][i]=inputPadded(b, c_in, y + yy + filter_size-1, x + i);
			}
			float error_val = error(b, c_out, y + yy, x);
			for (int j = 0; j < filter_size; j++) {
				for (int i =0; i < filter_size; i++) {
					val[j][i] += reuse_vals[j][i] * error_val;	
				}	
			}
...
template __global__ void  w_shfl_registers_reordered_padded_work_keep<3,1>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_shfl_registers_reordered_padded_work_keep<3,2>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_shfl_registers_reordered_padded_work_keep<3,4>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_shfl_registers_reordered_padded_work_keep<3,8>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_shfl_registers_reordered_padded_work_keep<3,16>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_shfl_registers_reordered_padded_work_keep<3,32>(Tensor<float, 4> inputPadded, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);