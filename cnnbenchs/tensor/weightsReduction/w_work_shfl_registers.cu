			...
			for (short i = (-filter_size+1)/2,ii =0; i <= filter_size/2; i++, ii++) {
				for ( int yy = 0; yy < yBatch; yy++){
					...
					for (short b = 0; b < batch_size; b++) {
						...
						val[jj][ii] += input_val * error(b, c_out, y + yy, x);
					}
				}
			}
		}
...
template __global__ void  w_work_shfl_registers<3,1>(Tensor<float, 4> input, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_work_shfl_registers<3,2>(Tensor<float, 4> input, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_work_shfl_registers<3,4>(Tensor<float, 4> input, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_work_shfl_registers<3,8>(Tensor<float, 4> input, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_work_shfl_registers<3,16>(Tensor<float, 4> input, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);
template __global__ void  w_work_shfl_registers<3,32>(Tensor<float, 4> input, Tensor<float, 4> error, Tensor<float, 4> gradient_weights);